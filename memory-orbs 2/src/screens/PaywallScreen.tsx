...
const PaywallScreen = ({ onClose }: PaywallScreenProps) => {
  ...
  const premiumFeatures = ['Unlimited memories', 'AI reflections', 'Extra themes', 'Export GIF'];
                ...
                <View style={styles.columns}>
                  <View style={[styles.tierColumn, styles.freeColumn]}>
                    ...
                    {freeFeatures.map((f) => (
                      <View key={f} style={styles.featureRow}>
                        <Text style={styles.checkFree}>{CHECK}</Text>
                  ...
                  <View style={[styles.tierColumn, styles.premiumColumn]}>
                    ...
                    {premiumFeatures.map((f) => (
                      <View key={f} style={styles.featureRow}>
                        <Text style={styles.checkPremium}>{CHECK}</Text>
...
const styles = StyleSheet.create({
  ...
  overlay: { flex: 1 },
  safeArea: { flex: 1, paddingHorizontal: 20, paddingTop: 4, paddingBottom: 20 },
  ...
  closeIcon: { color: 'rgba(255,255,255,0.6)', fontSize: 15 },
  ...
  freeColumn: { borderColor: 'rgba(255,255,255,0.08)' },
  premiumColumn: { borderWidth: 2, borderColor: '#5B6EF5' },
  tierTitle: { color: '#FFF', fontSize: 15, fontWeight: '700', marginBottom: 10 },
  featureRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8, gap: 6 },
  checkFree: { color: '#8A8FAD', fontSize: 13 },
  checkPremium: { color: '#5B6EF5', fontSize: 13 },
  freeFeatureText: { color: '#9CA2BE', flex: 1, fontSize: 13, lineHeight: 18 },
  premiumFeatureText: { color: '#D8DEFF', flex: 1, fontSize: 13, lineHeight: 18 },
  price: { color: '#FFF', textAlign: 'center', fontSize: 18, fontWeight: '700', marginBottom: 4 },
  altPrice: {
